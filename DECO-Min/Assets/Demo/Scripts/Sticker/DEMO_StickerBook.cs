...
/// </summary>
public class DEMO_StickerBook : MonoBehaviour
{
    public static DEMO_StickerBook Instance { get; private set; }
    ...
    [SerializeField] private List<DEMO_StickerData> startingStickers = new();
    [SerializeField] private int               startingCount    = 3;

    // ─── イベント ─────────────────────────────────────────────────────────────
    /// <summary>在庫が変化したとき（変化したシール, 新しい枚数）</summary>
    [HideInInspector] public UnityEvent<DEMO_StickerData, int> OnInventoryChanged      = new();
    /// <summary>選択中シールが変わったとき</summary>
    [HideInInspector] public UnityEvent<DEMO_StickerData>      OnSelectedStickerChanged = new();
    ...
    // ─── パブリックプロパティ ─────────────────────────────────────────────────
    public DEMO_StickerData SelectedSticker { get; private set; }
    ...
    public bool AddSticker(DEMO_StickerData sticker, int count = 1)
    {
    ...
    public bool ConsumeSticker(DEMO_StickerData sticker, int count = 1)
    {
    ...
    public void SelectSticker(DEMO_StickerData sticker)
    {
    ...
    public int GetStickerCount(DEMO_StickerData sticker)
    {
...
{
    public DEMO_StickerData Data;
    public int         Count;

    public StickerEntry(DEMO_StickerData data, int count)
    {
        Data  = data;
        Count = count;
    }
