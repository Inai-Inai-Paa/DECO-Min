...
/// All text in English.
/// </summary>
public class DEMO_StickerBookUI : MonoBehaviour
{
    // ─── Inspector ─────────────────────────────────────────────────────────────
    [Header("Grid")]
    ...
    private List<DEMO_StickerSlotUI>   slots = new();
    ...
    private void Start()
    {
        ...
        if (startButton != null)
            startButton.onClick.AddListener(() => DEMO_UIManager.Instance?.OnStartExplorationClicked());

        if (startButtonLabel != null)
            startButtonLabel.text = "Start Exploration";

        if (DEMO_GameManager.Instance != null)
            DEMO_GameManager.Instance.OnDayChanged.AddListener(d => UpdateDay(d));

        if (detailPanel != null) detailPanel.SetActive(false);

        UpdateDay(DEMO_GameManager.Instance?.CurrentDay ?? 1);
        Refresh();
    }
    ...
    public void Refresh()
    {
        ...
        foreach (var entry in sb.GetAllEntries())
        {
            ...
            var slot = go.GetComponent<DEMO_StickerSlotUI>();
            if (slot != null)
            {
    ...
    // ─── Detail panel ─────────────────────────────────────────────────────────
    private void ShowDetail(DEMO_StickerData data)
    {
