...
public sealed class CameraBlendState : CameraState
{
	...
	public override void Update()
	{
         ...
         * 各Volumeごとに
         *
         * origin + orbitRotation * offset
         *
         * を計算してから、その結果をブレンドする。
         */
		Vector3 targetCameraPosition =
			CalculateBlendedCameraPosition(
				orbitAngles,
				context.totalWeight);
	...
	/// <summary>
	/// 各BlendSampleのoffsetを、
	/// そのSample自身のbaseRotationに対するローカルオフセットとして扱い、
	/// FollowStateと同じ計算式でCamera位置を作ってからブレンドする。
	/// </summary>
	private Vector3 CalculateBlendedCameraPosition(
		Vector3 orbitAngles,
		float totalWeight)
	{
		if (totalWeight <=
		...
		Vector3 weightedCameraPosition =
			Vector3.zero;

		for (int i = 0;
			 i < _blendSamples.Count;
			 ++i)
		{
			BlendSample sample =
				_blendSamples[i];
		...
		out BlendContext context)
	{
		...
		Vector3 weightedZeroOrbitCameraPosition =
			Vector3.zero;
			 ...
			 ++i)
		{
			...
			/*
             * offsetは各VolumeのbaseRotationに対する
             * ローカルオフセットとして扱う。
             *
             * ここではEnter時の逆算用に、
             * Orbitなし状態の代表Camera位置を作る。
             */
			weightedZeroOrbitCameraPosition +=
				(sample.origin +
				 sample.baseRotation *
				 sample.offset) *
				sample.weight;
		...
		Vector3 blendedZeroOrbitCameraPosition =
			weightedZeroOrbitCameraPosition /
			totalWeight;

		Vector3 representativeOffset =
			Quaternion.Inverse(
				blendedBaseRotation) *
			(blendedZeroOrbitCameraPosition -
			 blendedOrigin);
	...
	private struct BlendContext
	{
		...
		/*
         * Enter時の現在Camera位置からOrbit角度を逆算するための
         * 代表ローカルオフセット。
         *
         * 実際のUpdateでは各Sampleごとにoffsetを解釈するため、
         * カメラ位置計算には使わない。
         */
		public Vector3 representativeOffset;
