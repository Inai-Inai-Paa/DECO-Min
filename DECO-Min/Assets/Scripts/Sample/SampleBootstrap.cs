...
public class SampleBootstrap : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var fos = FindObjectsByType<FieldObject>(FindObjectsSortMode.None);
        for (int i = 0; i < fos.Length; i++)
        {
            fos[i].RuntimeSetup(rewards[i % rewards.Length], count: 2);
            if (fos[i].GetComponent<FieldObjectIndicator>() == null)
                fos[i].gameObject.AddComponent<FieldObjectIndicator>();
        }
    ...
    private static StickerData Make(string name, Color color, StickerRarity rarity,
                                    int power, float force, float arc, string desc)
    {
        var d = ScriptableObject.CreateInstance<StickerData>();
